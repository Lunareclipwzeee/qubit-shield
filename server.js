...
app.post('/v1/sign', rateLimit(60, 60*1000),authenticate,async(req,res)=>{
  try{
    ...
    res.json({ok:true,signature:sig,algorithm:'HMAC-SHA256',standard:'Pending ML-DSA-65 — Node upgrade required'});
  }catch(err){res.status(400).json({ok:false,error:err.message});}
});
...
app.post('/v1/verify', rateLimit(60, 60*1000),authenticate,async(req,res)=>{
  try{
    ...
    res.json({ok:true,valid,algorithm:'HMAC-SHA256',standard:'Pending ML-DSA-65 — Node upgrade required'});
  }catch(err){res.status(400).json({ok:false,error:err.message});}
});
...
function requireSentinel(req,res,next){
  ...
  return res.status(403).json({ok:false,error:'EIGEN Sentinel requires Enterprise plan',upgrade:'mailto:pilotsqubitshield@gmail.com?subject=Upgrade to Enterprise'});
}
...
app.post('/platform/upgrade-request',async(req,res)=>{
  try{
    ...
    const payload=JSON.stringify({from:'EIGENLOCK <onboarding@resend.dev>',to:['pilotsqubitshield@gmail.com'],reply_to:'pilotsqubitshield@gmail.com',subject:`EIGENLOCK — Plan Upgrade Request: ${plan} from ${company}`,html});
